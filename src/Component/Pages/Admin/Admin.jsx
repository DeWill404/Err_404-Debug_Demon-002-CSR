            ...
            { csrID ? (
                dataTree && (dataTree==="---" || dataTree.username) ? (
                    <div className="container">
                        <title>CSR | Admin({dataTree && dataTree!=='---' ? dataTree.username : '---'})</title>
                        <Info />
                        ...
                        { dataTree!=='---' && Object.keys(dataTree).map(KEY => {
                            if (KEY !== 'username')
                                return ( compressed ? (
                                    showData(`${csrID}/${sessionID}`, compressed, dataTree, '', KEY, false)
                                ) : (
                                    <div key={KEY} className={'field-label rounded-2 px-1 my-2 border bg-label'}>
                                    <span className="d-flex label-text text-capitalize p-1" >
                                        ...
                                        <span
                                            ...
                                            data-placement="bottom"
                                            title={`Remove data`}
                                            onClick={ () => deleteData(`${csrID}/${sessionID}`, KEY) } >
                                                <i class="bi bi-trash"></i>
                                        </span>
                                    </span>
                                        { Object.keys(dataTree[KEY]).map(key => 
                                            showData(`${csrID}/${sessionID}`, compressed, dataTree[KEY], KEY, key, true)
                                        ) }
                                    </div>
                                ) )
                        }) }
                        
                        <button className="add-new btn btn-success w-100 fs-3 mt-2">+</button>
