        ...
        if (Array.isArray(tree[LABEL]))
            ...
            return (compressed ? (
                        Object.keys(tree[LABEL]).map( label => 
                            showData(id, compressed, tree[LABEL], getPath(path, LABEL), label, !bg) )
                    ) : (
                        <DetailsContainer
                        key={ getPath(path, LABEL) }
                        path={path}
...
function DetailsContainer(props) {
    ...
    return (
        <div className={`field-label px-1 pb-0 pe-0 ${!bg_color ? 'bg-label text-dark' : 'bg-label-white text-light'}`}
...
function DetailField(props) {
    ...
    return(
        <div className={`field-wrapper rounded-2 m-2 p-1 border ${validated ? 'bg-validate' : 'bg-invalidate'}`} >
