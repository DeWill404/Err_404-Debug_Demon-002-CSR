...
function showData(id, tree, path, LABEL) {
    ...
    if (Array.isArray(tree[LABEL])) {
        ...
        return Object.keys(tree[LABEL]).map(label => showData(id, tree[LABEL], `${path && path+' ⇒ '}${LABEL}`, label));
    }
}
...
function DataRow(props) {
    ...
    return(
        <div className={`field-wrapper row align-items-center rounded-2 my-2 ${validated ? 'bg-validate' : 'bg-invalidate'}`} >
