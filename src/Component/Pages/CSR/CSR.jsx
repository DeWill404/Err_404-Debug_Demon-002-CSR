    ...
    if (txt.innerHTML === 'Generate') {
        if (name) {
            ...
            if ( name.match(/^\w+$/g) ) {
                ...
                showErr("Username is invalid <br/> It should only contain <br/> Only Alphanumeric character <br/> No spaces anywhere <br/> No special character");
        ...
        { logged && logged.csr ? (
            <div>
                ...
                <div className="container">
                    <Info />
                    <span style={{fontFamily:"monospace", fontWeight:"bold"}}>{ sessions && `Hello, ${sessions.csr}`}</span>
                    ...
                    <div className="session-div p-2">
                        ...
                        <div className="row">
                            ...
                            <div className="col-6">
                                <Input type="text" name="Username" label="Username" refer={input} style={{maxWidth:"300px"}} />
                                <div>
                                    
                                    <button
                                        id="btn-submit"
                                        className="my-2 btn btn-primary align-item-center"
                                        onClick={(e) => generateSession(logged.csr, input.current.value, setKey, input)}>
                                            <span id="btn-txt" className="me-1">Generate</span>
                                            <span id="btn-spinner" className="spinner-border spinner-border-sm visually-hidden"></span>
                                    </button>
                                    ...
                                    {sessionKey && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            onClick={() => localStorage.setItem(props.login, logged.csr)}>
                    ...
                    <div className="session-div p-2">
                        ...
                        { sessions && Object.keys(sessions).map(key => {
                            return ( key!=='csr' &&
                                <SessionRow key={key} link={`${props.url}user/${key}`} obj={sessions[key]} />
                            );
                        }) }
                    </div>
                </div>
