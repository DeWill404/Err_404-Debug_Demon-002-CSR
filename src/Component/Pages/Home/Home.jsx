...
function switchMode(mode, setter) {
    ...
    if (!btn.classList.contains('btn-active')) {
        // Reset another btn
        document.getElementsByClassName('mode-btn')[mode === 0 ? 1 : 0].classList.remove('btn-active');
        // Set click btn
...
function login(mode, setlog, name, pass) {
    ...
    // If mode is CSR && username is entered
        ...
        checkCredential("user", name).then(snap => {

            // Check if detail are valid
            if (snap.val())
                setlog( {'user':snap.val()} );
            else
                setErrMSG("Invalid Details");
...
function Home(props) {
    ...
    useEffect(() => {
        ...
        if(logged) {
            ...
            history.push(logged.csr ? 'csr' : `user/${logged.user.substring(logged.user.indexOf('/')+1)}`);
        }
    });
        ...
        <main id="wrapper">
            ...
            <div className="login-card">
                ...
                <div className="form-section p-3 pt-1">
                    ...
                    { mode === 0 && <Input type="password" name="Password" label="Password" refer={upass} /> }
                    <button
                        className="mt-4 btn btn-primary align-item-center"
                        onClick={() => login(mode, setLog, uname.current?uname.current.value:'', upass.current?upass.current.value:'')}>
