...
function disableReload(func) {
    window.onbeforeunload = () => { return "Reloading or Leaving page will close the session" };
    window.onunload = () => {
...
var sep = ' ⇒ ';
const getPath = (path, label) => {return `${path ? path+sep+label : label}`};
...
function setLabelHeight(name) {
    ...
    // Get input list
    const elements = name ? [document.querySelector(`input[name=${name}]`)] : document.getElementsByClassName('input-model');
    for (let i = 0; i < elements.length; i++)
        changeHeight(elements[i]);
}
