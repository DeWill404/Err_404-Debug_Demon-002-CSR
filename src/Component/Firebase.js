...
function checkCredential(type, userName) { return db.ref(`${type}/${userName}`).get() }
...
function registerUser(uname, key) { db.ref(`user/${uname}`).set(key) }
...
function unregisterUser(id) {
	db.ref(`session/${id}`).get().then(snap => {
		// Unregister user
		const data = snap.val();
		if (data)
			Object.keys(data).map(key => removeUser(data[key].username));
		// Delete database
...
function switchBool(id, label, index, value) { db.ref(`session/${id}/${label}/${index}`).set(!value) }

// Funtion to update data in input fields
function updateData(id, label, value) { db.ref(`session/${id}/${label}/0`).set(value) }

// Function remove data in from input field
function deleteData(id, path) { db.ref(`session/${id}/${path}`).remove() }

// Function cleas data in from input field, save as empt object
function clearData(id, path) { db.ref(`session/${id}/${path}`).set("") }
